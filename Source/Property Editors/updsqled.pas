...
type
  ...
  TUpdateSQLEditForm = class(TForm)
    OkButton: TButton;
...
uses Dialogs, BdeConst, LibHelp, TypInfo, BDE;

{ Global Interface functions }

function EditUpdateSQL(AUpdateSQL: TUpdateSQL): Boolean;
begin
  with TUpdateSQLEditForm.Create(Application) do
  try
...
function TUpdateSQLEditForm.DatabaseOpen: Boolean;
begin
...
function TUpdateSQLEditForm.Edit: Boolean;
var
  Index: TUpdateKind;
  DataSetName: string;
begin
...
procedure TUpdateSQLEditForm.GenWhereClause(const TabAlias, QuoteChar: string;
  KeyFields, SQL: TStrings);
var
  I: Integer;
  BindText: string;
...
procedure TUpdateSQLEditForm.GenDeleteSQL(const TableName, QuoteChar: string;
  KeyFields, SQL: TStrings);
begin
  SQL.Clear;
...
procedure TUpdateSQLEditForm.GenInsertSQL(const TableName, QuoteChar: string;
  UpdateFields, SQL: TStrings);
...
procedure TUpdateSQLEditForm.GenModifySQL(const TableName, QuoteChar: string;
  KeyFields, UpdateFields, SQL: TStrings);
var
  I: integer;
  Comma: string;
...
procedure TUpdateSQLEditForm.GenerateSQL;
...
procedure TUpdateSQLEditForm.GetDataSetFieldNames;
begin
...
procedure TUpdateSQLEditForm.GetTableFieldNames;
begin
...
function TUpdateSQLEditForm.GetTableRef(const TabName, QuoteChar: string): string;
begin
  if QuoteChar <> '' then
    Result :=  TabName + '.' else
    REsult := '';
...
procedure TUpdateSQLEditForm.InitGenerateOptions;
var
  UpdTabName: string;
...
procedure TUpdateSQLEditForm.InitUpdateTableNames;
begin
...
procedure TUpdateSQLEditForm.SetButtonStates;
begin
...
procedure TUpdateSQLEditForm.SelectPrimaryKeyFields;
var
...
procedure TUpdateSQLEditForm.SetDefaultSelections;
var
...
procedure TUpdateSQLEditForm.ShowWait(WaitMethod: TWaitMethod);
begin
...
function TUpdateSQLEditForm.TempTable: TTable;
begin
...
procedure TUpdateSQLEditForm.FormCreate(Sender: TObject);
begin
  HelpContext := hcDUpdateSQL;
end;

procedure TUpdateSQLEditForm.HelpButtonClick(Sender: TObject);
begin
  Application.HelpContext(HelpContext);
end;

procedure TUpdateSQLEditForm.StatementTypeClick(Sender: TObject);
begin
...
procedure TUpdateSQLEditForm.OkButtonClick(Sender: TObject);
begin
  if SQLMemo.Modified then
    SQLText[TUpdateKind(StmtIndex)].Assign(SQLMemo.Lines);
end;

procedure TUpdateSQLEditForm.DefaultButtonClick(Sender: TObject);
begin
  with UpdateTableName do
...
procedure TUpdateSQLEditForm.GenerateButtonClick(Sender: TObject);
begin
  GenerateSQL;
  FSettingsChanged := False;
end;

procedure TUpdateSQLEditForm.PrimaryKeyButtonClick(Sender: TObject);
begin
  ShowWait(SelectPrimaryKeyFields);
  SettingsChanged(Sender);
end;

procedure TUpdateSQLEditForm.PageControlChanging(Sender: TObject;
  var AllowChange: Boolean);
begin
...
procedure TUpdateSQLEditForm.FormDestroy(Sender: TObject);
begin
  if DatabaseOpened then
    Database.Session.CloseDatabase(Database);
end;

procedure TUpdateSQLEditForm.GetTableFieldsButtonClick(Sender: TObject);
begin
...
procedure TUpdateSQLEditForm.SettingsChanged(Sender: TObject);
begin
...
procedure TUpdateSQLEditForm.FormCloseQuery(Sender: TObject;
...
procedure TUpdateSQLEditForm.UpdateTableNameChange(Sender: TObject);
begin
  SettingsChanged(Sender);
end;

procedure TUpdateSQLEditForm.UpdateTableNameClick(Sender: TObject);
begin
  if not Visible then Exit;
  GetTableFieldsButtonClick(Sender);
end;

procedure TUpdateSQLEditForm.SelectAllClick(Sender: TObject);
begin
  SelectAll(FieldListPopup.PopupComponent as TListBox);
end;

procedure TUpdateSQLEditForm.ClearAllClick(Sender: TObject);
var
  I: Integer;
begin
...
procedure TUpdateSQLEditForm.SQLMemoKeyPress(Sender: TObject;
  var Key: Char);
begin
