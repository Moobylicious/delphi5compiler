...
var
  ...
  var
    ...
    Dummy: DWORD;
  begin
    VerInfoSize := GetFileVersionInfoSize(PChar(IBASE_DLL), Dummy);
    GetMem(VerInfo, VerInfoSize);
    GetFileVersionInfo(PChar(IBASE_DLL), 0, VerInfoSize, VerInfo);
    VerQueryValue(VerInfo, '\', Pointer(VerValue), VerValueSize);
    Result := VerValue^.dwFileVersionMS shr 16;
...
begin
  ...
  begin
    ...
    //GW - we need to skip this check, as it now picks up firebird 1.x or 2.x as interbase < 6, so various things
    //stop working with IBX 5.04!  
//    IBClientVersion := GetVersion;
