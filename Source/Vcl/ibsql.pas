...
uses
  IBIntf, IBBlob, IBSQLMonitor, IBXMLHeader, DBConsts;
  ...
  if not IsNull then
    case FXSQLVar^.sqltype and (not 1) of
      ...
      SQL_DOUBLE, SQL_FLOAT, SQL_D_FLOAT:
        result := FloatToStr(AsDouble);
      SQL_BOOLEAN:
        if PShort(FXSQLVAR^.sqldata)^ = ISC_TRUE then
          Result := STextTrue
        else
          Result := STextFalse;
      else
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if j_len + aliasname_length > (METADATALENGTH - 1) then
              StrPCopy(aliasname,
                       Copy(st, 1, (METADATALENGTH - 1) - j_len) +
                       IntToStr(j))
            else
              StrPCopy(aliasname, st + IntToStr(j));
          end;
