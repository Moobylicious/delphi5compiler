...
type
  ...
  private
    ...
    FEditorMode: Boolean;
    //Added by GW, to prevent accidental compilation of Aligrid.Pas against wrong (i.e. original) version of
    //grids.pas from Borland.
    FPlaysafeCompatible : Boolean;
  ...
  protected
    ...
    property PlaysafeCompatible : boolean read FPlaysafeCompatible;
  public
    constructor Create(AOwner: TComponent); override;
...
var
  ...
  begin
    ...
    begin
      ...
      //Don't bother doing anything if Bottom is above top!
      //This would normally be because the row has been hidden by setting RowHeights to -1.
      //This If statement was added by GW 17/11/2010.  The one below that effectively does exactly the same
      //but for hidden columns (Where.Right > Where.Left) was already here.  I would suggest that this is
      //an oversight by Borland, and it causes issues in our (Playsafe Custom component) TStringAligGrid
      //component, in 'Aligrid.pas' which is normally in the components\Shared source folder, and it built into the
      //'Shared' package.  TStringAlignGrid inherits from TStringGrid which inherits from TDrawGrid which inherits from
      //TCustomDrawGrid, so the only other way to resolve this would be to create a new TPlaysfeCustomGrid, TPlaysafeDrawGrid,
      //and TPlaysafeStringGrid, and inherit TStringAlignGrid from that... and that's a LOT of duplicated code for a a 3-line
      //fix (including a begin and end!).  So I'll put this comment in....
      if (Where.Bottom > Where.Top) then
      begin
...
procedure TCustomGrid.WMLButtonDown(var Message: TMessage);
begin
  inherited;
...
procedure TCustomGrid.CMCancelMode(var Msg: TMessage);
begin
