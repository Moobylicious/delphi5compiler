    ...
    {  Check first for computed fields, then domains.
      ...
      begin
        ...
        begin
          ...
          if FieldType = blr_boolean_dtype then
            Column := Column + ColumnTypes[14].TypeName;  {do not localize}
...
begin
  ...
  try
    ...
    begin
      ...
      Line := Format('CREATE DOMAIN %s AS ', [QuoteIdentifier(FDatabase.SQLDialect, FieldName)]);  {do not localize}
  ...
  try
    ...
    begin
      ...
      FMetaData.Add(Format('CREATE EXCEPTION %s %s%s',  {do not localize}
        [QuoteIdentifier(FDatabase.SQLDialect, qryException.FieldByName('RDB$EXCEPTION_NAME').AsTrimString), {do not localize}
...
begin
  ...
  try
    ...
    begin
      Line := Format('ALTER TABLE %s ADD ', [QuoteIdentifier(FDatabase.SQLDialect, {do not localize}
      ...
      if (not qryForeign.FieldByName('RELC1_CONSTRAINT_NAME').IsNull) and   {do not localize}
        ...
        Line := Line + Format('CONSTRAINT %s ', [QuoteIdentifier(FDatabase.SQLDialect,  {do not localize}
...
const
  IndexSQL =
    ...
    '       IDX.RDB$INDEX_TYPE ' +     {do not localize}
...
begin
  ...
  try
    ...
    begin
      ...
      SList.Add(Format('CREATE VIEW %s (', [QuoteIdentifier(FDatabase.SQLDialect, {do not localize}
      ...
      while not qryColumns.Eof do
      begin
        SList.Add('  ' + QuoteIdentifier(FDatabase.SQLDialect,    {do not localize}
           qryColumns.FieldByName('RDB$FIELD_NAME').AsTrimString));  {do not localize}
        qryColumns.Next;
        if not qryColumns.Eof then
  ...
  try
    ...
    begin
      ...
      if FieldName = '' then
        ...
        ColString := Format(' (%s)', [QuoteIdentifier(FDatabase.SQLDialect, FieldName)]); {do not localize}
    ...
    begin
      ...
      begin
        ...
        FMetaData.Add(Format('GRANT %s TO %s%s%s',      {do not localize}
          [QuoteIdentifier(FDatabase.SQLDialect, qryOwnerPriv.FieldByName('RDB$RELATION_NAME').AsTrimString),  {do not localize}
  ...
  try
    ...
    begin
      ...
      FMetaData.Add(Format('GRANT %s TO %s%s%s%s',    {do not localize}
        [ QuoteIdentifier(FDatabase.SQLDialect, qryRole.FieldByName('RDB$RELATION_NAME').AsTrimString),  {do not localize}
...
var
  ...
  begin
    Result := Format('  %s ', [QuoteIdentifier(FDatabase.SQLDialect, qryHeader.FieldByName('RDB$PARAMETER_NAME').AsTrimString)]); {do not localize}
    for i := Low(ColumnTypes) to High(ColumnTypes) do
  ...
  try
    ...
    for i := 0 to qrySelect.Current.Count - 1 do
      ...
      begin
        ...
        if not(qrySelect.Database.Has_DEFAULT_VALUE(ObjectName, FieldName)) then
        begin
          if Fields <> '' then
            Fields := Fields + ', '; {do not localize}
          Fields := Fields + QuoteIdentifier(FDatabase.SQLDialect, FieldName);
        end;
      end;
    while not qrySelect.Eof do
    begin
      Line := 'INSERT INTO ' + QuoteIdentifier(FDatabase.SQLDialect, ObjectName) + ' (';  {do not localize}

      Line := Fields + ') VALUES (';  {do not localize}
      Values := '';
      for i := 0 to qrySelect.Current.Count - 1 do
      begin
        if not(qrySelect.Database.Has_DEFAULT_VALUE(ObjectName, FieldName)) then
